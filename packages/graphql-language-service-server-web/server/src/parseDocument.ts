...
import { CachedContent, Range, Position } from "graphql-language-service";
import { findGraphQLTags } from "./findGraphQLTags";

type TagResult = { tag: string; template: string; range: Range };
const DEFAULT_TAGS = ["graphql", "graphqls", "gql"];
...
export function parseDocument(
  ...
  if (fileExtensions.includes(ext)) {
    ...
    const templates: TagResult[] = findGraphQLTags(text, ext, uri, logger);
